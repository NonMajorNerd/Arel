        ...
        for i in range(number_of_monsters):
            ...
            if not any([entity for entity in entities if entity.x == x and entity.y == y]):
                ...
                if monster_choice == 'rat':
                    ...
                    monster = Entity(x, y, 305, libtcod.light_gray, 'bat', blocks=True,
                                     render_order=RenderOrder.ACTOR, fighter=fighter_component, ai=ai_component)
                    ...
                    monster = Entity(x, y, 295, libtcod.desaturated_green, 'goblin', blocks=True,
                                     render_order=RenderOrder.ACTOR, fighter=fighter_component, ai=ai_component)
                else:
                    fighter_component = Fighter(hp=30, defense=2, power=8, speed=5, xp=100)
                    ai_component = BasicMonster()

                    monster = Entity(x, y, 296, libtcod.light_orange, 'Troll', blocks=True, fighter=fighter_component,
                ...
                if item_choice == 'healing_potion':
                    ...
                    item_component = Item(use_function=cast_fireball, targeting=True, targeting_message=Message(
                        'Left-click a target tile for the fireball, or right-click to cancel.', libtcod.light_cyan),
                                          damage=25, radius=3)
                    item = Entity(x, y, 333, libtcod.red, 'Fireball Scroll', render_order=RenderOrder.ITEM,
                                  item=item_component)
                elif item_choice == 'confusion_scroll':
                    item_component = Item(use_function=cast_confuse, targeting=True, targeting_message=Message(
                        'Left-click an enemy to confuse it, or right-click to cancel.', libtcod.light_cyan))
                    item = Entity(x, y, 333, libtcod.light_pink, 'Confusion Scroll', render_order=RenderOrder.ITEM,
                                  item=item_component)
                else:
                    item_component = Item(use_function=cast_lightning, damage=40, maximum_range=5)
                    item = Entity(x, y, 333, libtcod.yellow, 'Lightning Scroll', render_order=RenderOrder.ITEM,
                                  item=item_component)
