...
class vendor_data_loader:
    ...
    def populate_vendor_inventory(): #watcha hawkin? (I'll figure out a better way to handle this later, for now it works for implementation n testing)
        ...
        #staff
        item_component_staff = Item(use_function=None, stackable=False, count=100, cost=5,
        ...
        #sword
        item_component_sword = Item(use_function=None, stackable=False, count=100, cost=10,
        ...
        #cure wounds
        item_component_cure_wounds = Item(use_function=game_map.heal, stackable=False, amount=20, count=5, cost=10,
        ...
        numWares = numWares + item_component_staff.count + item_component_sword.count + item_component_cure_wounds.count
...
def vendor_menu(player, vendor):
    ...
    if True:
        ...
        libtcod.console_print_ex(0, 42, 3, libtcod.BKGND_NONE, libtcod.LEFT, "Gold: " + str(player.current_gold)) 
    ...
    while True:
        ...
        numitems = len(vendor.inventory.items)
        ...
        if numitems > 0:
            ...
            for x in range (start, end):
                if x < len(_globals.vendorEnt.inventory.items):
                    itm = get_item_at("inventory", currentpage, x, _globals.vendorEnt)
        ...
        # global item
        item = get_item_at(system, currentpage, iindex, _globals.vendorEnt)
        ...
        if item.equippable:
            ...
            comp = item
            # for eq in vendorEnt.inventory.list:
        ...
        libtcod.console_print_ex(0, 31, 30, libtcod.BKGND_SET, libtcod.LEFT, "Current Gold: " + str(player.current_gold))
...
def buy_item(player, item, quantity):
    results = []

    #libtcod.console_print_ex(0, 6, 5, libtcod.BKGND_NONE, libtcod.LEFT, "Welcome, weary contestant, to my humble shop.")

    if player.current_gold >= item.item.cost: #if you have the gold, take the gold and give the item
        ...
        for i in player.inventory.items:
            if i.name == item.name and i.item.stackable:
                ...
                libtcod.console_print_ex(0, 6, 9, libtcod.BKGND_NONE, libtcod.LEFT, "Purchased " + item.name + " x " + item.count + " ")
                libtcod.console_flush()
                results.append({
                    'item_added': item,
